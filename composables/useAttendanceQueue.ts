...
const runTransaction = async <T>(mode: IDBTransactionMode, execute: (store: IDBObjectStore, setResult: (value: T) => void) => void) => {
  ...
  return await new Promise<T>((resolve, reject) => {
    ...
    try { execute(store, setResult) }
    catch (error) { tx.abort(); reject(error) }
  })
}

export const useAttendanceQueue = () => {
  const pendingCount = useState('pending-attendance-count', () => 0)
  const flushing = useState('attendance-flushing', () => false)
  ...
  const putMany = async (mutations: AttendanceMutation[]) => {
    if (!mutations.length || !import.meta.client || !('indexedDB' in window)) return
    await runTransaction<void>('readwrite', (store) => { mutations.forEach((mutation) => store.put(mutation)) })
    await refreshCount()
  }

  const put = async (mutation: AttendanceMutation) => await putMany([mutation])
    ...
    try {
      ...
      while (queued.length) {
        ...
        const response = await $fetch<{ accepted: string[] }>('/api/summer/attendance/batch', {
          method: 'POST',
          headers: { 'x-summer-device-id': batch[0]?.deviceId || 'anonymous' },
  ...
  return { pendingCount, flushing, list, put, putMany, remove, flush, refreshCount }
}
