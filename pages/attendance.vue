...
const visibleDays = computed(() => days.value.flatMap((day) => {
  const rows = day.rows.filter((row) => {
    if (scope.campus.value !== 'all' && row.campus !== scope.campus.value) return false
...
const groupedRows = (rows: AttendanceHistoryDay['rows']) => (['Toluca', 'Metepec'] as CampusName[]).flatMap((campus) => {
  const students = rows.filter((row) => row.campus === campus)
...
<template>
  <div class="page-container history-page">
    <section class="page-heading">
      <div><span>Summer Camp 2026</span><h1>Historial de asistencia</h1></div>
    ...
    <section class="range-card">
      ...
      <button class="primary-button" :disabled="busy" @click="load">Consultar</button>
    </section>
    ...
    <div v-else-if="visibleDays.length" class="history-list">
      <article v-for="day in visibleDays" :key="day.date" class="history-day">
        <button class="history-day__summary" @click="expanded = expanded === day.date ? null : day.date">
          <div><span>{{ formatDate(day.date) }}</span><strong>{{ day.total }} registros</strong></div>
    ...
    <div v-else class="empty-state"><img src="/icons/abejas.png" alt=""><strong>Sin registros en el periodo</strong></div>
  </div>
</template>
