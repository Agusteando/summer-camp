...
const addColumnIfMissing = async (table: string, column: string, definition: string) => {
  const rows = await appQuery<any[]>(`SHOW COLUMNS FROM \`${table}\` LIKE ?`, [column])
  if (!rows.length) await appQuery(`ALTER TABLE \`${table}\` ADD COLUMN \`${column}\` ${definition}`)
}

const addIndexIfMissing = async (table: string, index: string, definition: string) => {
  const rows = await appQuery<any[]>(`SHOW INDEX FROM \`${table}\` WHERE Key_name = ?`, [index])
  if (!rows.length) await appQuery(`ALTER TABLE \`${table}\` ADD ${definition}`)
}

export default defineNitroPlugin(async () => {
  const config = useRuntimeConfig()
  if (!config.appMysqlHost) return
  try {
    await appQuery(`CREATE TABLE IF NOT EXISTS summer_student_overrides (
    ...
    await appQuery(`CREATE TABLE IF NOT EXISTS summer_attendance (
    ...
    await addColumnIfMissing('summer_attendance', 'device_id', "VARCHAR(80) NOT NULL DEFAULT 'anonymous' AFTER actor_name")
    await addIndexIfMissing('summer_attendance', 'idx_summer_attendance_device', 'KEY idx_summer_attendance_device (summer_year, device_id, attendance_date)')
  } catch (error) {
    console.error('[summer migrations]', error)
  }
})
